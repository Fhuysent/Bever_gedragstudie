...
HRdata <- spTransform(HRdata, BEL)

##95%MCP
##om een nog niet duidelijke reden geeft enkel km input en m2 output de juiste output in ha
cp <- mcp(HRdata[,1], percent=95, unin = "m", unout = "ha")

##Oppervlakten
Oppervlaktes  <- as.data.frame(cp)
```

```{r percentage, fig.cap="Evolutie ", echo=FALSE}
mcpopp <- mcp.area(HRdata[,1], percent=seq(50, 100, by = 5))
```

```{r MCPkaart, fig.cap="Home ranges voor de 14 bevers met geschikte data, berekend via MCP.", echo=FALSE}
pal <- colorFactor(palette = polychrome(14) , domain = cp@data$id)

leaflet()%>%
  addTiles()%>%
  addPolygons(data=cp, color=~pal(id), fillOpacity = 1, stroke = F)
```
