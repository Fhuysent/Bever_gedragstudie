...
Bevers met een zender konden gemiddeld `r round(mean(Zendertabel$dagen), 0)` dagen worden opgevolgd, variërend van `r min(Zendertabel$dagen)` tot `r max(Zendertabel$dagen)`. Dit leverde gemiddeld  `r round(mean(Zendertabel$Aantal), 0)` (`r min(Zendertabel$Aantal)`-`r max(Zendertabel$Aantal)`) datapunten per bever op. Dit betekent dat gemiddeld elke `r Zendertabel %>% mutate(opbrengst = Aantal/dagen) %>% summarise(effectiviteit = round(1/mean(opbrengst),digits=1))` dagen een lokalisatie kon worden bepaald.

Al deze gegevens zijn informatief maar voor de gedetailleerde verwerking wordt enkel gebruik gemaakt van gegevens van bevers die voldoende gegevens opleverden. Omdat de gegevens verder per maand zullen worden geanalyseerd werd ervoor geopteerd om voor deze detailanalyses enkel die bevers te weerhouden die minstens een volledige maand konden worden opgevolgd. Gezien het gemiddelde van `r Zendertabel %>% mutate(opbrengst = Aantal/dagen) %>% summarise(effectiviteit = round(1/mean(opbrengst),digits=1))` dagen tussen twee lokalisaties, werd daarbij geopteerd om ook een minimum van 10 verzamelde datapunten als ondergrens voor gedetailleerde verwerking te hanteren.
...
kable(TabelHR,
  ...
  col.names = c("Bever", "Oppervlakte (95%MCP) (ha)", "Rivierlengte (km)")
 )
```
...
###Overzicht meandergebruik casussen {#h:meanders}

Fig. \@ref(fig:figmeanders) geeft de verdeling van waarnemingen binnen en buiten de meanders weer per maand voor elk van de drie, hierboven besproken, territoria (\@ref(h:terr1), \@ref(h:terr2), \@ref(h:terr3)). Daarin is duidelijk, dat, los van de ongelijke verdeling van het totaal aantal waarnemingen per maand, de meanders in elk seizoen een belangrijk onderdeel van de territoria vormen. Het feit dat in elk van deze territoria ook de hoofdburchten waar de voortplanting plaatsvindt, zich op een meander bevond onderstreept deze vaststelling. Enkel in het territorium op de Dijle tussen Haacht en Rijmenam (\@ref(h:terr2)), waar de meander in de zomermaanden tot in de herfst grotendeels droog komt te liggen, valt op dat het gebruik van deze meander door de aanwezige bevers enkele maanden totaal wegvalt. Het feit dat dit ook het duidelijk wordt in de gegevens van de maand oktober, waarin het aantal observaties wel zeer hoog was, toont aan dat deze vaststelling geen artefact als gevolg van beperkte beschikbare gegevens is.
