...
#  mutate(Bever_ID = as.integer(Bever_ID)) %>% 
  ...
  left_join(BeverIDs[,c("bever", "VHF.kort","Einddatum")], by = c("Bever_ID" = "VHF.kort")) %>% 
  mutate(Einddatum = if_else(is.na(Einddatum), Sys.Date(), Einddatum)) %>% 
  filter(Datum<Einddatum)
...
TelemetrieMap <- Telemetrie %>% 
 filter(y<51.13 & y>51.11) %>% 
  ...
  filter(bever %in% c('BE1130','BE1131'))
...
leaflet(data=TelemetrieMap) %>% 
  addTiles() %>%
  addCircleMarkers(~x,~y,
                   color=~pal(bever),stroke = F, radius=5, fillOpacity = 0.8) %>% 
