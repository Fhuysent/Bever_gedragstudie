...
for (i in fileNames) {
  datasetname <- regmatches(i, gregexpr("(?<=MyMaps/).*(?=.kml)", i, perl=T))[[1]]
  assign(datasetname, read_kml_file(i))
  }


tmp<-Sporen
Sporen <- tmp
Sporen$Datum <- as.numeric(Sporen$Datum)/1000000
Sporen$Datum <- as.POSIXct(Sporen$Datum, origin="1970-01-01")
