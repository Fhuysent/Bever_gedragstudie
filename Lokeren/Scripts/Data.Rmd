...
Hier worden de gegevens ingelezen de Lotek PinPoint Host vanaf de GPS zenders worden ingeladen. Deze gegevens zijn beschikbaar als .txt bestanden.

```{r data inlezen GPS, include=FALSE}
GPS <- read.delim("../Input/GPS/PinPoint 44030 2024-03-25 11-11-14_V8_113_0.txt", header = TRUE, sep ="") %>% 
  mutate(date = as.Date(RTC.date, format = "%y/%m/%d"))
...
```{r kaartje GPS, include=FALSE}
leaflet(data=GPS) %>% 
  addTiles() %>%
  addCircleMarkers(~Longitude,~Latitude,stroke = F, radius=5, fillOpacity = 0.9) %>%
